...
export async function getAllComponents(page = 1, pageSize = "all") {
  return await promisifiedDataFetch(
    `${COMPONENTS_ENDPOINT}?page=${page}&pagesize=${pageSize}`,
  );
}

export async function getMeshModels(
  page = 1,
  pageSize = "all",
...
export async function getComponentFromModelApi(
  model,
  pageSize = "all",
  trim = true,
) {
...
export async function getMeshModelsByRegistrants(
  page = 1,
  pageSize = "all",
  registrant,
) {
...
export async function getRelationshipFromModelApi(
  model,
  pageSize = "all",
  trim = true,
) {
...
export async function getDuplicateComponents(
  componentKind,
  apiVersion,
  modelName,
) {
...
export async function getMeshModelComponent(
  model,
  component,
  version,
  apiVersion,
...
export async function getModelFromCategoryApi(category) {
  return promisifiedDataFetch(
    `${CATEGORIES_ENDPOINT}/${category}/models?pagesize=all`,
  );
}
...
export async function searchModels(queryString, options = defaultOptions) {
  return promisifiedDataFetch(
    `${MESHMODEL_ENDPOINT}?search=${encodeURI(
      queryString,
    )}&${optionToQueryConvertor({
      ...defaultOptions,
      ...options,
    })}`,
  );
...
export async function searchComponents(queryString, options = defaultOptions) {
  return promisifiedDataFetch(
    `/api/meshmodels/components?search=${encodeURI(
      queryString,
    )}&${optionToQueryConvertor(options)}`,
  );
}
