...
export function isMeshSyncSubscriptionDataUpdated(currentState, newData) {
  ...
  const oldData = currentState.find(
    (data) => data.contextID === newData.contextID,
  );
  if (!oldData) {
    return true;
  }
...
export function isMesheryControllerStateSubscriptionDataUpdated(
  currentState,
  newData,
) {
  console.log(
  ...
  console.log(
    "CONTROLLER OLD DATA NEW DATA AFTER COMPARATOR",
    oldData,
    newData,
  );
  return !_.isEqual(oldData, newData);
